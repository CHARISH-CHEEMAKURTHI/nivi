...
func _build_place_bar(root: Control) -> void:
	...
	row.add_child(_button("Place", "GreenButton", func() -> void: request_place_confirm.emit()))
	row.add_child(_button("Cancel", "RedButton", func() -> void: request_place_cancel.emit()))
	_place_bar.add_child(row)

func show_place_bar(type: String) -> void:
	_place_label.text = "Placing %s" % Config.BUILDINGS[type]["name"]
...
func show_help() -> void:
	...
	for line in [
		"Drag to pan the island, scroll or pinch to zoom. WASD pans too.",
		"Tap a building to inspect it. Mines fill up over time, so tap them or press Collect.",
		"Build, then pick a building, drag it where you want it and press Place.",
